...
def Landsat():
    ...
    # Select the seven NLCD epoches after 2000.
    region = ["Kab. Bandung Barat 2013", "Kab. Bandung Barat 2021", "Kab. Purwakarta"]

    # Get an NLCD image by year.
    def getRS(region):

        if region == "Kab. Bandung Barat 2013":
            Map.set_center(lat=-6.920803423087094, lon=107.4476469379034, zoom=10)
            ee_path = 'users/bills/Bandung_Barat'
        ...
        elif region == "Kab. Bandung Barat 2021":
            Map.set_center(lat=-6.920803423087094, lon=107.4476469379034, zoom=10)
            ee_path = 'users/bills/Bandung_Barat'
            west_bandung = ee.FeatureCollection(ee_path)
            west_bandung = west_bandung.geometry()
    ...
    if selected_region:
        ...
        st.subheader("Confusion Matrix")
        if region == "Kab. Bandung Barat 2021":
            st.image("https://i.ibb.co/mJSMd59/CM-Bandung-Barat-Land8-1.png")
        else:
            pass
